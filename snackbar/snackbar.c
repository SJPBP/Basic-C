...
void add_items(void)
{
    ...
    for (int i = 0; i < NUM_ITEMS; i++)
    {
        ...
        menu[i].item  = get_string("Provide Name for Item %i: ", i);
        if (strlen(menu[i].item) == 0)
        {
            printf("\n");
            break;
        }
        menu[i].price = get_float("Provide Price for Item %i: ", i);
...
float get_cost(string item)
{
    ...
    // loop through each item from menu
    for (int j = 0; j < NUM_ITEMS; j++)
    {
        // check if element from item is in menu
